var wxTimer = function(initObj) {
  ...
  this.timeToStart = initObj.timeToStart || 0; // 秒
  this.endDateTime = initObj.endDateTime; // 秒
...
wxTimer.prototype = {
  //开始
  start: function(self) {
    const timezonePadding = (new Date(Date.now()).getTimezoneOffset()) * 60 * 1000; // 获取时差

    this.endTime = this.timeToStart * 1000 + timezonePadding; //1970年1月1日的00：00：00的字符串日期
    this.endSystemTime = new Date(this.endDateTime);
    ...
    function begin() {
      ...
      self.setData({
        ...
        wxTimerFormat: {
          dd: tmpTime.getDate() - 1,
          hh: tmpTimeStr.substr(0, 2),
          mm: tmpTimeStr.substr(3, 2),
          ss: tmpTimeStr.substr(6, 2)
        }
      });
      //时间间隔执行函数
